...
const Quiz = ({ data, countdownTime, endQuiz, onQuite }) => {
  ...
  const handleItemClick = (e, ansData) => {
    try {
      ...
      setQuestionsAndAnswers((p) => {
        ...
        let qData = [
          ...p?.filter(
            (ele) => ele?._id !== he?.decode(data[questionIndex]?._id)
          ),
          {
  ...
  const handleSkip = (e) => {
    ...
    setQuestionsAndAnswers((p) =>
      p.filter((ele, i) => ele?._id != currQState?._id)
    );
              ...
              <Item.Content>
                <Item.Extra className="text-center flex justify-between items-center flex-wrap  flex-row-reverse">
                  ...
                  <div className="m-auto  mb-3 lg:mb-0 flex flex-col justify-center items-center gap-4">
                    ...
                    {numberOfPausesLeft > 0 && (
                      <Button
                        ...
                        sx={{
                          backgroundColor: `rgb(203 215 225 / var(--tw-bg-opacity))`,
                        }}
                        variant="outlined"
                        endIcon={
                          currentLang == `en` ? null : (
                  ...
                  <div className="m-auto flex gap-6 items-end justify-center flex-wrap items-center ">
                    ...
                    <button
                      onClick={handleQuite}
                      class="ui inverted red button transition-all"
                    >
                      {t(`Quite`)}
                    </button>
                  </div>
                ...
                <Item.Meta>
                  <Message
                    size={`${screenWidth > 500 ? 'huge' : 'large'}`}
                    floating
                  >
                    <b
                  ...
                  <Menu
                    vertical
                    fluid
                    size={`${screenWidth > 500 ? 'massive' : 'large'}`}
                  >
                    {/* .filter((option) => {
                        const id = option;
                        if (!uniqueOptions[id]) {
                    ...
                    {data[questionIndex].options.map((option, i) => {
                      try {
                        ...
                        {
                          /* console.log(option, `decoded => `, he?.decode(option)); */
                        }
                        {
                          /* console.log(option); */
                        }
                        ...
                        return (
                          <Menu.Item
                            ...
                            className="font-bold"
                            style={{
                              fontWeight: screenWidth < 500 ? 700 : 400,
                            }}
                ...
                >
                  {!(questionIndex + 1 == data?.length) ? (
                    ...
                    <div
                      className={
                        'ml-auto flex  gap-3 ' + currentLang == `ar` &&
