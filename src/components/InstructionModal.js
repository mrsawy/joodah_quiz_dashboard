...
export default function BasicDemo({ className, isQuizStarted }) {
  ...
  useLayoutEffect(() => {
    (async () => {
      ...
      const value = result?.find(
        (item) => item.identifier == `instructions`
      )?.value;
      const valueAr = result?.find(
        (item) => item.identifier == `instructions-ar`
      )?.value;
      if (currentLang == `ar`) {
        setText(valueAr);
  ...
  return (
    <div
      className={'card flex justify-content-center  ml-3 sm:ml-0' + className}
    >
      <Button
